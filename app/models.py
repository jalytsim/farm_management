...
class GFWLog(db.Model):
    """
    Journal des visites et téléchargements liés aux rapports GFW / Carbon.

    action_type :
      'page_view'             — ouverture du composant Carbon Report
      'pdf_download'          — téléchargement du PDF rapport
      'certificate_generated' — génération d'un certificat EUDR
    entity_type : 'farm' | 'forest' | 'geojson_upload'
    """
    ...
    def __repr__(self):
        return f'<GFWLog {self.action_type} | {self.entity_type}:{self.entity_id}>'


class SMSLog(db.Model):
    """
    Journal de tous les SMS envoyés via /api/notifications/sms.
...
class SentinelCache(db.Model):
    """
    Cache des données Sentinel-2 par ferme.

    Logique :
      - 1 ligne par farm_id
      - Mise à jour automatique si stale_after est dépassé (90 jours)
      - history_json   : données historiques 5 ans (liste de dicts)
      - forecast_json  : forecast Prophet 4 trimestres (dict par indice)
      - Les données sont stockées compressées en JSON dans LONGTEXT

    stale_after : date à partir de laquelle les données doivent être
                  rafraîchies (calculée = created_at + 90 jours)
    """
    __tablename__ = 'sentinelcache'

    id           = db.Column(db.Integer, primary_key=True)
    farm_id      = db.Column(db.String(50), nullable=False, unique=True, index=True)
    history_json = db.Column(LONGTEXT, nullable=True)   # JSON historique
    forecast_json= db.Column(LONGTEXT, nullable=True)   # JSON forecast Prophet
    ltv_json     = db.Column(db.Text,  nullable=True)   # JSON LTV (petit)
    period_from  = db.Column(db.String(20), nullable=True)
    period_to    = db.Column(db.String(20), nullable=True)
    stale_after  = db.Column(db.DateTime, nullable=True)  # date d'expiration
    created_at   = db.Column(db.DateTime, default=datetime.utcnow)
